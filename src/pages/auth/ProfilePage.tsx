...
const ProfilePage: FC = () => {
  ...
  if (isError || !user) {
    return (
      <div className="container mx-auto py-10 text-center bg-background">
        <p className="text-destructive">Gagal memuat profil. Silakan <Link to="/login" className="underline">login</Link> kembali.</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            >
              {isEditingProfile ? <XCircle className="w-5 h-5 text-destructive" /> : <Edit2Icon className="w-5 h-5 text-foreground" />}
            </Button>
          </CardHeader>
          <CardContent>
            {isEditingProfile ? (
        ...
        <Card>
          ...
          <CardContent>
            <Form {...passwordForm}>
              <form onSubmit={passwordForm.handleSubmit(data => changePasswordMutate(data))} className="space-y-4 text-muted-foreground">
